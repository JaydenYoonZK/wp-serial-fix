...
- The name seal is now a proper square chop. The characters and inner frame are cut clean through the chartreuse field, so whatever the bar shows behind it fills the strokes and the seal reads right in light and dark mode alike. The brand name next to it grew a touch to hold its own.
...
- The film grain overlay was pinned to the viewport, and a full-bleed fixed layer makes iOS Safari composite scrolled page content into the status bar strip, so text ghosted around the clock and the WiFi icons. The grain now sits on the document itself and scrolls with it, which keeps that strip clean. On the page it looks exactly as before.

## [1.4.10] - 2026-08-16

### Fixed

- The green pill that floated over an iPhone's status bar is gone. It was the skip-to-content link, resting just above the viewport's top edge, exactly the zone iOS paints behind the clock. It parks off to the side now and still drops in on keyboard focus. Chased down and confirmed in the iOS Simulator instead of guessed at.

### Changed

- The nav bar is pinned with position fixed instead of sticky, so a hard fling can never lag it a frame, and it carries safe-area padding so it owns the strip under the status bar when the page is saved to the home screen. The shields from the last two releases are removed: both tried to paint over the status-bar zone, and the Simulator settled it, nothing inside a page draws there. The faint content visible behind the status bar mid-scroll is Safari's own edge-to-edge design, apple.com shows it identically, and this changelog will not fight it again.
...
- The copyright sentence stands alone on its own line again, with the version link on its own line right beneath it.
...
[1.3.6]: https://github.com/JaydenYoonZK/wp-serial-fix/releases/tag/v1.3.6
[1.3.5]: https://github.com/JaydenYoonZK/wp-serial-fix/releases/tag/v1.3.5
